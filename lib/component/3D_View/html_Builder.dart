...
abstract class HTMLBuilder {
  ...
  static String build({
    ...
    final defaultCameraOrbit = cameraOrbit ?? "0deg 75deg 35m";
    // camera-orbit
    html.write(' camera-orbit="${htmlEscape.convert(defaultCameraOrbit)}"');
    ...
    html.write('const modelViewer = document.querySelector("model-viewer");');

    html.write("modelViewer.addEventListener('loop', ()=>{");
    html.write('sendBack("2");');
    html.write('});');

    html.write('function reset() {');
    html.write('const orbit = modelViewer.getCameraOrbit();');
    html.write('modelViewer.resetInteractionPrompt();');
    html.write('modelViewer.resetTurntableRotation(orbit.theta);');
    html.write('modelViewer.resetTurntableRotation(0);');
    html.write('modelViewer.cameraOrbit = `$defaultCameraOrbit`;');
    html.write('modelViewer.pause();');
    html.write('modelViewer.currentTime = 0;');
    html.write('}');

    html.write('function play() {');
    html.write('modelViewer.play({repetitions: 40});');
    html.write('}');

    html.write('function zoom(val) {');
    html.write('const orbitStr = modelViewer.getCameraOrbit().toString();');
    html.write("const orbitList = orbitStr.split(' ');");
    html.write("orbitList.pop();");
    html.write("orbitList.push(val + 'm');");
    html.write("modelViewer.cameraOrbit =  orbitList.join(' ');");
    html.write('}');

    html.write('function pause() {');
    html.write('modelViewer.pause();');
    html.write('}');

    html.write('function isLoaded() {');
    html.write('sendBack(modelViewer.modelIsVisible);');
    html.write('}');

    html.write('function sendBack(val) {');
    html.write('messageHandler.postMessage(val);');
    html.write('}');
    html.write('</script>');
